...
def compute_kozo_metrics(financials: pd.DataFrame) -> pd.DataFrame:
    ...
    * ``sga_vs_peer`` -- Requires Premium ``/fins/fs_details`` for SGA
    ...
    if not supplement.empty:
        logger.info(
            "Kozo supplement: %d rows, columns=%s",
            len(supplement), list(supplement.columns),
        )
        logger.info(
            "Kozo supplement index (first 5): %s",
        ...
        if code_col is not None:
            ...
            logger.info(
                "Kozo supplement code match: %d / %d financials codes found in supplement index",
                matched, len(code_col_norm),
            )
    else:
        logger.warning("Kozo supplement is empty — no Sheets data available.")
    ...
    if not supplement.empty and code_col_norm is not None:
        ...
        for metric_col in _DIRECT_METRIC_COLS:
            ...
            logger.info(
                "Kozo supplement %-20s: %d values filled, %d non-null after merge",
                metric_col, filled,
        ...
        if "ke" in supplement.columns:
            raw_ke = code_col_norm.map(supplement["ke"])
